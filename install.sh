...
TIMES="10"
CHATID="6617783693"
KEY="ZhuqeyUOdYFc6742Q1YUVF1EjM"
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/xawn22/db/main/check/database.ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/xawn22/db/main/check/database.ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/xawn22/db/main/check/database.ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/xawn22/db/main/check/database.ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/xawn22/db/main/check/database.ip | grep $ipsaya | awk '{print $3}')
...
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ","url":"https://wa.me/6282240074362"},{"text":"ɪɴꜱᴛᴀʟʟ","url":"https://t.me/LunaticTunnel"}]]}'
...
wget https://sky.myawan.web.id/file/limit.sh && chmod +x limit.sh && ./limit.sh
...
wget -O /usr/sbin/sky "${REPO}file/sky" >/dev/null 2>&1
chmod +x /usr/sbin/sky
